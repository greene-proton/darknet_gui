...
import gui.printdragon as dragon

class YOLOv3MainGUI(object):
    """main darknet class, showing option for starting training and testing commands"""
    static_array = []
    def __init__(self):
        self.dyn_array = []
        self.window = []

    def init_gui_1_layout(self):
        """return GUI layout"""
        layout = [
    ...
    def init_darknet_gui(self):
        """initialise darknet interface gui"""
        sg.ChangeLookAndFeel('GreenTan')
        while True:
            window = self.init_gui_1_layout()
            ...
            if event == 'Train':
                ...
                if not values[0] or not values[1] or not values[2] or not values[3] or not values[4]:
                    ...
                    if not os.path.isdir(values[1]):
                        ...
                        sg.Popup('WARNING: *.data not found or invalid directory:\n{}'.format(values[2]))
                    elif not os.path.isfile(values[3]):
                        sg.Popup('WARNING: *.cfg not found or invalid directory:\n{}'.format(values[3]))
                    elif not os.path.isfile(values[4]):
                        sg.Popup('WARNING: *.weights not found or invalid directory:\n{}'.format(values[4]))
                    else:
